...
const TimePicker = React.forwardRef<TimePickerRef, TimePickerProps>(
  ({ date, onChange, hourCycle = 24, granularity = "second" }, ref) => {
    ...
    const [period, setPeriod] = React.useState<Period>(internalDate.getHours() >= 12 ? "PM" : "AM");

    React.useEffect(() => {
      const formatted = format(internalDate, "HH:mm:ss");
      onChange?.(formatted as any); // Cast to expected type if onChange expects string
    ...
    return (
      <div className="flex items-center justify-center gap-2 border-2 border-zinc-600 rounded-md px-2">
        ...
        <TimePickerInput
          ...
          onDateChange={(newDate) => setInternalDate(newDate ?? new Date())}
          ref={hourRef}
          period={period}
